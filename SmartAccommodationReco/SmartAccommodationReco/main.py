...
def precision_recall_ndcg_at_k(train_ui, test_ui, recommender, k=10):
    ...
    for u in users:
        ...
        if not gt:
            continue
        rec_items = recommender(set(train_ui.get(u, set())))

        hits = [1 if it in gt else 0 for it in rec_items]
        tp = sum(hits)
        prec = tp / max(1, len(rec_items))
        rec  = tp / max(1, len(gt))
...
# =====================================
# đoạn gọi
precision, recall, ndcg = precision_recall_ndcg_at_k(train_ui, test_ui, recommender, k)
